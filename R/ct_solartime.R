  ...
  if (!is.null(data)) {
    ...
    for (loc in 1:length(unique_location)) {
      ...
      completed <- dplyr::bind_rows(completed, tibble(!!dplyr::sym(lon_str) := longitude,
                                                      ...
                                                      solar = ct_transtime(tm, suntimes)))
    }
...
#' @noRd
ct_wrap <- function (x, bounds = c(0, 2 * pi)) {
  bounds[1] + (x - bounds[1])%%diff(bounds)
  }

#' @noRd
ct_get_time <- function (x, scale = c("radian", "hour", "proportion"), ...,
...
#' @noRd
ct_transtime <- function (date, anchor,
                          mnanchor = NULL,
  ...
  if (type == "single") {
    res <- ct_wrap(mnanchor[1] + date - anchor[, 1])
  }
  else {
    difs <- ct_wrap(cbind(date, date) - anchor)
    flip <- difs[, 1] > difs[, 2]
    ...
    res <- ct_wrap(baseline + interval * relpos)
  }
  res
}
