...
#' @param traprate_data A data frame created by [ct_traprate_data()].
#' @param radius_model Optional. A detection function model for radius (distance) fitted using [ct_fit_detmodel()].
#' @param angle_model Optional. A detection function model for angle fitted using [ct_fit_detmodel()].
#' @param speed_model Optional. A model for movement speed fitted using [ct_fit_speedmodel()].
...
#'
#' @seealso [ct_fit_speedmodel()], [ct_fit_detmodel()], [ct_fit_activity()]
#'
#' @export
ct_fit_rem <- function(data,
                       traprate_data,
                       radius_model = NULL,
                       angle_model = NULL,
                       speed_model = NULL,
  ...
  if(is.null(radius_model))
    radius_model <- ct_fit_detmodel(radius~1, data, order = 0, truncation = "5%")
  cli::cli_process_done()

  cli::cli_process_start("Fitting angle model")
  if(is.null(angle_model))
    angle_model <- ct_fit_detmodel(angle~1, data, order = 0, unit = "radian")
  cli::cli_process_done()

  cli::cli_process_start("Fitting speed model")
  if(is.null(speed_model))
    speed_model <- ct_fit_speedmodel(formula = speed~1, data = data)
  cli::cli_process_done()
...
#'
#' @seealso [ct_fit_rem()], [ct_fit_detmodel()], [ct_fit_activity()]
#'
#' @export
ct_fit_speedmodel <- function(formula = speed ~ 1,
                              data,
                              newdata = NULL,
...
#'
#' @seealso [ct_fit_rem()], [ct_fit_speedmodel()], [ct_fit_activity()]
#'
#' @export

ct_fit_detmodel <- function(formula,
                            data,
                            newdata = NULL,
...
get_parameter_table <- function (traprate_data,
  ...
  rownames(res) <- c("radius", "angle", "active_speed", "activity_level",
                     "overall_speed")
  traprate <- ct_traprate_estimate(traprate_data, strata, reps)
