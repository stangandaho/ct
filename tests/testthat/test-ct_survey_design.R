test_that("test survey design", {
  ...
  random_sdes <- ct_survey_design(study_area = pendjari_trans, method = "random", verbose = T,
                                  total_site = 15, min_distance = 5000, padding = 2000,
                                  set_seed = 123)
  testthat::expect_true(inherits(random_sdes, "sf"))

  ##
  regular_sdes <- ct_survey_design(study_area = pendjari_trans, method = "regular", verbose = T,
